...
func NewReportServer(msgModel *messages.Model, serverAddress string) error {
	...
	s := grpc.NewServer()
	api.RegisterReportServer(s, &server{msgModel: msgModel})
...
func (s *server) SendReport(ctx context.Context, request *api.ReportRequest) (*api.ReportReply, error) {
	err := s.msgModel.SendReport(ctx, request.GetReport(), request.GetUserID())
	if err != nil {
		return nil, errors.Wrap(err, "cannot send report")
	}
	return &api.ReportReply{Message: "successfully sent"}, nil
}
